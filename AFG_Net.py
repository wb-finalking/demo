...
class VGG(object):
    ...
    def buildNet(self, netName, images, num_classes, dropout_keep_prob=0.5,
                 is_training=False, final_endpoint='conv5'):
...
class CRNN(tf.contrib.rnn.RNNCell):
    def __init__(self, conv_ndims, input_shape, output_channels, kernel_shape,
        ...
        self._state_size = input_shape + [output_channels]
        self._output_size = input_shape + [output_channels]
...
class AFGNet(object):
    ...
    def buildNet(self, images, num_classes, weight_decay=FLAGS.vgg_weight_decay,
        ...
        with tf.variable_scope('BCRNN'):
            with slim.arg_scope([slim.conv2d],
                ...
                heat_maps = tf.sigmoid(heat_maps, scope='sigmoid')
            ...
            with tf.variable_scope('RK_2'):
                grammar_serial_RK2 = tf.stack([heat_maps[0], heat_maps[1]], axis=3)
                grammar_serial_RK2 = tf.transpose(grammar_serial_RK2, (0, 3, 1, 2))
                RK2_refined_heatmaps = self.multiLayerBidirectionalRnn(1, 3, grammar_serial_RK2, 2)
                RK2_refined_heatmaps = tf.transpose(RK2_refined_heatmaps, (1, 0, 2, 3))

            with tf.variable_scope('RK_3'):
                grammar_serial_RK3 = tf.stack([heat_maps[4], heat_maps[6], heat_maps[7]], axis=3)
                grammar_serial_RK3 = tf.transpose(grammar_serial_RK3, (0, 3, 1, 2))
                RK3_refined_heatmaps = self.multiLayerBidirectionalRnn(1, 3, grammar_serial_RK3, 3)
                RK3_refined_heatmaps = tf.transpose(RK3_refined_heatmaps, (1, 0, 2, 3))

            with tf.variable_scope('RK_4'):
                grammar_serial_RK4 = tf.stack([heat_maps[4], heat_maps[5]], axis=3)
                grammar_serial_RK4 = tf.transpose(grammar_serial_RK4, (0, 3, 1, 2))
                RK4_refined_heatmaps = self.multiLayerBidirectionalRnn(1, 3, grammar_serial_RK4, 2)
                RK4_refined_heatmaps = tf.transpose(RK4_refined_heatmaps, (1, 0, 2, 3))

            with tf.variable_scope('RS_1'):
                grammar_serial_RS1 = tf.stack([heat_maps[0], heat_maps[4]], axis=3)
                grammar_serial_RS1 = tf.transpose(grammar_serial_RS1, (0, 3, 1, 2))
                RS1_refined_heatmaps = self.multiLayerBidirectionalRnn(1, 3, grammar_serial_RS1, 2)
                RS1_refined_heatmaps = tf.transpose(RS1_refined_heatmaps, (1, 0, 2, 3))

            with tf.variable_scope('RS_2'):
                grammar_serial_RS2 = tf.stack([heat_maps[1], heat_maps[5]], axis=3)
                grammar_serial_RS2 = tf.transpose(grammar_serial_RS2, (0, 3, 1, 2))
                RS2_refined_heatmaps = self.multiLayerBidirectionalRnn(1, 3, grammar_serial_RS2, 2)
                RS2_refined_heatmaps = tf.transpose(RS2_refined_heatmaps, (1, 0, 2, 3))

            with tf.variable_scope('RS_3'):
                grammar_serial_RS3 = tf.stack([heat_maps[2], heat_maps[6]], axis=3)
                grammar_serial_RS3 = tf.transpose(grammar_serial_RS3, (0, 3, 1, 2))
                RS3_refined_heatmaps = self.multiLayerBidirectionalRnn(1, 3, grammar_serial_RS3, 2)
                RS3_refined_heatmaps = tf.transpose(RS3_refined_heatmaps, (1, 0, 2, 3))

            with tf.variable_scope('RS_4'):
                grammar_serial_RS4 = tf.stack([heat_maps[3], heat_maps[7]], axis=3)
                grammar_serial_RS4 = tf.transpose(grammar_serial_RS4, (0, 3, 1, 2))
                RS4_refined_heatmaps = self.multiLayerBidirectionalRnn(1, 3, grammar_serial_RS4, 2)
                RS4_refined_heatmaps = tf.transpose(RS4_refined_heatmaps, (1, 0, 2, 3))

            # max merge heatmaps
            l_collar = tf.maximum([grammar_serial_RK1, grammar_serial_RK1, grammar_serial_RK1])
                # l - sleeve
                # l - waistline
                # l - hem
            output = tf.transpose(output, (0, 2, 3, 1))
            output = tf.nn.softmax(output, name='RefinedHeatMaps')
